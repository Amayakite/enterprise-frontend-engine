...
右上角“外观设置 → 界面密度”提供更紧凑、紧凑、默认、宽松、更宽松五档，
即时作用于页面留白、表单字段、详情、主子表与公共弹窗正文。
切换只改变显示，不重新加载实体、清除草稿或移动保存操作。
紧凑模式缩减留白，保留字段说明、错误信息和操作入口；表格的换行仍由 `wrapCells` 独立决定。
窄屏限制页面外边距，触摸设备保留弹窗按钮的操作面积。

布局遵循两类尺度，不把所有间距等比放大：

- 随密度调整：字段之间、数据行内部、正文分区与面板内边距；面板留白上限 24px，避免宽松档挤占正文。
- 保持固定：菜单行与弹窗标题栏 44px、窗口操作区上下 8px、按钮组间距 8px、详情标签到值 6px、标题到状态 8px。
  分区导航按钮至少 32px 高；图标、边框、圆角、地图画布和 Word 纸张不随密度缩放。
  字段输入仍使用 Element Plus 的公开 small/default/large 尺寸，保留五档密度的输入体验差异。

公共内容尺度维护在 `src/assets/styles/density.scss`，组件使用 `--ui-panel-padding`、
`--ui-section-gap`、`--ui-field-gap`、`--ui-table-cell-padding` 等变量。
导航及工具区采用固定的组件间距，不复用字段/分区 token 推算点击区域。
详情单列折叠依据组件实际容器宽度，窄抽屉和窄屏使用同一原则。
`MyForm`、`MyDesc` 和 `MyTable` 的 `density` 省略时继承全局，
显式 `compact/comfortable` 仅用于确有需要的局部字段留白，不改变全局设置。

密度沿用主题的 `useStorage` 保存到当前浏览器。首次读取兼容原“布局大小”：
小型对应紧凑，大型对应宽松，其他对应默认。列表旧列偏好中的密度保留兼容读取，
标准列表不再使用它控制界面；恢复列默认不会重置全局密度。此设置不承诺账号隔离或跨端同步。

Word 合同模板使用 `<MyDialog :body-padding="false" fill-height body-scroll="content">`
建立贴边工作区；`bodyPadding` 默认 true，普通弹窗继续使用全局正文留白。
变量通过工具栏“插入变量”按需展开，底部保留导出和应用；应用后仍需保存公司档案。
界面密度不会修改 Word 正文字号、纸张或导出排版。
