...
`MyDialog` 默认桌面可拖拽、窄屏禁用拖拽，支持全屏/还原并在关闭后复位；默认 footer 提供确认/取消和确认禁用原因，也可用 `footer` 完整替换，`actions` 用于标题栏扩展。参照、查询、系统维护、个人资料、通知、服务申请和子表行编辑等普通生产弹窗均已接入。命令面板保留专用 `el-dialog`，因为它是无标题栏、点击遮罩关闭且自行管理输入焦点/键盘导航的全局特殊浮层；组件实验页的直接用法仅用于嵌套焦点探针。`MyCrudForm` 默认只在顶部显示保存/关闭，`footer` 仅在业务显式提供时作为额外底部区域渲染。
