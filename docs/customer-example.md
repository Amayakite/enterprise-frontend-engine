...
入口：开发服务登录后，基础资料 → 客户管理（`/#/base/customer`）。这是新模块职责划分的主要参考：列表、表单和详情使用 CRUD 装配；联系人与地址各有独立配置。无子表模块沿用主模块结构并精简子表装配。按文件展开的代码见 [模块开发实操](./module-development-example.md)。
...
src/pages/base/customer/
  ...
  types.ts                   页面模型、上下文及继承公共合同的 CustomerContract
  children/types.ts          客户专属的子配置合同与定义辅助函数
  children/contacts/config.ts 联系人字段、行键、校验、主要项动作和 DTO 映射
  children/contacts/CustomerContacts.vue 联系人编辑/只读呈现装配
  children/addresses/config.ts 地址字段、行键、校验、默认项动作和 DTO 映射
  children/addresses/CustomerAddresses.vue 地址编辑/只读呈现装配
```
...
客户 add/edit 各自通过 useCrudView 装配完整表单，模板直接使用 MyCrudForm，不共用 Editor；页面保留场景差异，模型、校验、保存和草稿复用公共配置/控制器。选择规则见 [业务模块开发规范](./business-module-standard.md)。
...
结构参考不等于照抄全部实现：`defineCustomerChildConfig` 是客户专属而非公共 API；主子表关系则复用公共 defineAggregateBinding，`org-a` 是 Mock 范围，行 ID 和整单保存也受当前 DTO 约束。新模块应按真实合同调整；默认主字段和归属直接可见；确实复杂时才按维护收益拆分片。

## 页面配置与扩展

`config.page` 集中提供 basePath、组织来源、列偏好版本、布局列数和 Mock 提示。
`useCrudView(customerModule, { view })` 内部复用 useBusinessPage 装配 context、navigation、preference、draftIdentity；
...
保留 dialog/drawer 打开模式；定制布局接法见 [CRUD 指南](./crud-development-guide.md#新增修改的标签页弹窗抽屉)。
...
- `page.add/edit` 默认 tab；把 mode 改为 drawer 或 dialog 即可试用相应入口。index/detail 已装配宿主，保存后刷新当前列表/详情。
- 查看结果、失败原因、请求编号和不确定提交提示均为公共 UI。操作的权限、全量筛选、幂等、持久化仍须真实后端落实。

完整接法与合同以 [CRUD 指南](./crud-development-guide.md) 字典、批量与页面呈现章节为准。
