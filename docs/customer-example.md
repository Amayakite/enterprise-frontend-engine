...
在客户列表勾选当前页客户，打开“已选 N 项 · 操作 → 批量修改销售组织”。
弹窗显示客户信息、原销售组织，右侧固定“调整后销售组织”，每位待审核客户可选择不同的组织。
已审核客户只读，沿用先撤销审核再修改的规则。清空后不能保存；取消会丢弃弹窗内的修改。

实现见 [CustomerSaleDialog.vue](../src/pages/base/customer/CustomerSaleDialog.vue)：

- `MyDialog + MyTable` 提供定高弹窗与列表；字段生成列后，只给销售组织补充 `fixed: "right"`。
- `column-saleId` 插槽放 `FieldInput`，复用销售组织参照 field。输入始终显示，`change` 按客户 ID 直接更新弹窗副本，无需进入 MyTable 行编辑。
- 没有开启表格 `edit`，所以该例自行检查必填、审核状态和权限，不依赖 `form.rules` 自动执行。
- 点击保存只提交变化行的 `id/version/saleId`。Mock `POST /api/v1/pilot/customers/sales` 检查组织范围、记录版本、审核状态和销售组织可用性，整批通过后才更新，其他客户资料和子表保持原值。
- 保存后刷新列表并标记客户缓存过期；请求结果未知时保留输入、禁止重提，提示关闭并刷新核实。

这只是开发 Mock 示例，不代表正式后端批量接口、鉴权、事务和持久化已完成。
