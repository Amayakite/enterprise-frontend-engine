...
入口：开发服务登录后，任务管理 → 任务费用（`/#/task/fee`）。本文记录现有实现，仅作金额、批量动作和保存回执回读等局部能力参考，不作为新模块结构范本。新模块统一参考 [客户管理](./customer-example.md) 和 [模块开发实操](./module-development-example.md)；公共能力接口约定见 [CRUD 开发指南](./crud-development-guide.md)。
...
- 金额使用 Decimal 校验与格式化，不经 `number`；附件沿用 `FileInfo[]`，状态沿用字典接口约定。
- `CrudAction` 统一处理权限、可见性、禁用原因、确认、单飞和刷新。保存后按 ID 再读详情；失败保留输入，回填失败只能 `retrySync()`。
- 菜单、隐藏路由和按钮使用 `task:fee:*` 权限；费用金额、附件、依赖回填和业务文案留在费用模块。

当前接口是 `/api/v1/pilot/task-fees` 开发接口约定，写入只保存在 Mock 进程内存。真实后端仍需联调 URL、数据权限、金额与时区、附件、版本冲突、批量部分失败、事务、取消、幂等及提交状态查询。
