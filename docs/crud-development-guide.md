...
`pnpm bench:crud` 对真实 `useFormModel` 执行预热和 100 次受控回写，分别输出 0/100/500 行
...
`pnpm test:e2e` 运行固定浏览器流程；首次运行按 Playwright 提示安装 Chromium。
测试使用独立开发 Mock 服务，不复用日常开发数据；具体端口和启动命令见根目录
`playwright.config.ts`。失败时保留截图/trace，产物在 `artifacts/playwright/`，不提交产物。
