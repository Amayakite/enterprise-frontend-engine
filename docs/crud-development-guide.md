...
主字段通过 `MyForm` 的 `patch.changes` 增量回写控制器；未变化的子表引用保持稳定，
不因为输入主字段重建子表数据。普通 `MyForm v-model` 仍发布完整模型，但不保证每次更新
都深复制未变化的分支；需要长期保留独立快照时使用公共 `cloneModel/readonlyModel`。
表单内部、发布模型与初始化基线相互隔离；字段联动回调和异步保存钩子仍使用隔离快照。
...
标准 `MyCrudForm` 支持 Ctrl+S / macOS Command+S。仅焦点在该表单内部时处理，
复用保存按钮的权限、只读、草稿、子表准备状态以及字段异步处理限制；输入法组合输入与按键连发不提交。
快捷键先让当前输入完成 change，再走原控制器整单保存。焦点在独立子表弹窗、参照或其他弹层时，
不会冒泡保存背后的父表单；后台 KeepAlive 页面也不注册全局保存监听。

按改动范围从以下场景选择相关项，不要求每次遍历全部场景：
...
验证命令与执行范围统一见 [README](../README.md#快速开始)。浏览器直接操作本次改动的实际业务路径，必要时检查键盘焦点、弹层返回和窄屏。交付时说明实际验证结果、未验证项、Mock 写入只在开发服务进程内保留以及真实后端尚待联调的协议。
...
验收页编辑记录后可装入 100/500 行测试明细，再检查主字段连续输入、子表分页、
“制造末页明细错误”后的定位与修正。按钮替换当前未提交明细，只用于独立测试记录。

排查性能时可选用 `pnpm bench:crud`，不作为日常验收门槛。它对真实 `useFormModel` 执行预热和 100 次受控回写，分别输出 0/100/500 行
下的中位数和 P95，比较兼容原位修改与 CRUD 不可变输入。该结果不包含 DOM、网络或图表，
不能解读成浏览器帧率；机器负载会影响数值，不设置固定毫秒门槛。
命令还重复创建/关闭 100 个 500 行模型实例，输出 GC 后堆增量趋势；它不包含 DOM/显存，
不能单独证明浏览器没有内存泄漏，应结合请求取消、订阅注销和浏览器反复切页验证。

运行 `pnpm dev` 后，通过内置浏览器或浏览器 MCP 检查对应页面即可，无需在项目安装浏览器测试框架。
涉及保存、删除时使用独立测试记录；需要保留问题截图时放在 `artifacts/`，不提交产物。
