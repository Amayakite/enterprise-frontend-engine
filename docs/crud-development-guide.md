...
行为测试命令是 `node --test tests/*.test.mjs`；公共合同或页面模型变更运行 `pnpm type-check`，跨模块或构建相关改动运行 `pnpm build`。浏览器验证必须操作实际业务路径。记录实际命令、结果和日期，并说明 Mock 写入只在开发服务进程内保留以及真实后端尚待联调的协议。
