<template>
  ...
  >
    <template #column-primary="{ row }">
      <el-tag v-if="binding.readonly && row.primary" size="small" effect="plain">默认</el-tag>
      ...
      <el-radio
        ...
        :aria-label="`设为默认地址 ${row.label || '新地址'}`"
        @change="setPrimary(row.id)"
      >
        <span class="sr-only">默认</span>
      </el-radio>
    </template>
    <template #empty><span class="customer-child-empty">暂无收货地址，可按需添加</span></template>
  </MyCrudChildTable>
  <MyTable
    v-else
    :rows="rows"
  ...
  >
    <template #column-primary="{ row }">
      <el-tag v-if="row.primary" size="small" effect="plain">默认</el-tag>
      <span v-else>—</span>
    </template>
    <template #empty><span class="customer-child-empty">暂无收货地址</span></template>
  </MyTable>
</template>
...
import { customerAddressesConfig as module } from "./config";

const props = defineProps<{
  binding?: CrudTableBinding<CustomerAddress>;
  rows?: readonly CustomerAddress[];
}>();
const rows = computed(() =>
  props.binding
    ? cloneReadonlyModel<CustomerAddress[]>(props.binding.rows)
