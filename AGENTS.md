...
3. 按用户当前需求实现最小改动；规范不是待办，不顺带重写旧模块。示例解释用法，实际接口约定核对源码。
...
- JSDoc 是交付要求：先说明有什么用、怎么用、传什么和得到什么，再解释内部实现。公共参数说明允许值、默认/空值和示例；回调说明入参、返回及影响，覆盖项说明替换哪一步。@example 独立成行。写法见 [注释规范](./docs/typescript-and-structure.md#注释先回答使用问题)；改动需核验实际 TS 悬停，不能只补 README。
...
- 按 [README](./README.md#快速开始) 选择最小必要验证：TS/Vue 改动做类型检查，逻辑改动跑相关测试；浏览器直接验证交互，不默认新增测试依赖。不要每次跑全量测试和构建，构建包含类型检查，无需重复执行。
